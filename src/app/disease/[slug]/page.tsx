            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {linkedTests.map((test) => (
                <Link
                  key={test.slug}
                  href={`/test/${test.slug}`}
                  className="group flex items-start gap-3 border border-gray-200 rounded-xl p-4 hover:border-purple-300 hover:bg-purple-50 transition-all"
                >
                  <div className="w-8 h-8 rounded-lg bg-purple-100 flex items-center justify-center flex-shrink-0">
                    <FlaskConical size={14} className="text-purple-600" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm text-gray-900 group-hover:text-purple-700 transition-colors">
